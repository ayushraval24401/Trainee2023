...
namespace CustomerManagement.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public ActionResult Register(User model)
        {
            ...
            try
            {
                var existingUser = _usersCollection.Find(u => u.Email == model.Email).FirstOrDefault();
        ...
        public ActionResult Login(Login model, [FromServices] IConfiguration configuration)
        {
            ...
            try
            {
                ...
                var user = _usersCollection.Find(u => u.Email == model.Email && u.Password == model.Password).FirstOrDefault();
                if (user != null)
                {
                    objResponse.access_token = CommonMethods.GenerateJSONWebToken(user, secretKey.Value, ValidAudience.Value, ValidIssuer.Value);
                    objResponse.status = 1;
