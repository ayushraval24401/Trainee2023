$(document).ready(function () {
    $('#example').DataTable({
        order:[],
        ajax: {
            url: 'EmployeeData_10042023.json',
            dataSrc: ''
        },
   ...
   // $('[data-bs-toggle="tooltip"]').tooltip();
    ...
    $('body').on('click', '.fa-eye', function() {
        var id = $(this).data('id');
        // make an AJAX call to fetch the details of the employee with the given ID
        // populate the modal with the data using jQuery
        $('#datatableModal').modal('show'); // show the modal
        $.ajax({
            url: 'EmployeeData_10042023.json',
            dataType: 'json',
            success: function(data) {
                ...
                if (employee) {
                    ...
                    // Color code the department text based on its value
                   var departmentText = employee.Department;
                   var color;
                   ...
                   break;
                    ...
                    $('#department').text(departmentText).css('color', color);
